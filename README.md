...
<br />
<div align="center">
  <a href="https://github.com/ikhsan892/goodin">
    <img src="docs/logo.png" alt="Logo" width="80" height="80">
  </a>
  <h3 align="center">GoOdin</h3>
  <p align="center">
    The Allfather of Go Development.
    <br />
    <br />
    <br />
    <a href="https://github.com/ikhsan892/goodin">View Demo</a>
    ·
    <a href="https://github.com/ikhsan892/goodin/issues">Report Bug</a>
    ·
    <a href="https://github.com/ikhsan892/goodin/issues">Request Feature</a>
  </p>
</div>
...
- [Migration](#migration)
- [Protobuf](#protobuf)
  - [Generate DTO for Use Case](#generate-dto-for-use-case)
- [Nats](#nats)
...
- [NATS server & nats CLI](https://docs.nats.io/) (Optional)
- [Protobuf](https://developers.google.com/protocol-buffers)
- Mockery
- Make tools (build-essentials) ([tutorial for Windows](https://medium.com/@samsorrahman/how-to-run-a-makefile-in-windows-b4d115d7c516))

## Available Commands

1. `make run/dev`: Run dev commands.
2. `make run/message-broker`: Run message broker driver only.
3. `make test`: Run tests for all packages.
4. `make test/tiultemplate`: Run tests with race checker.
5. `make swagger/init`: Generate Swagger docs.
6. `make test/cover`: Run test coverage.
7. `make run/http`: Run REST API HTTP driver.
8. `make run/migration`: Run migrations.
9. `make run/seeders`: Run seeders.
10. `make run/live`: Run dev commands with live reloads.

## Folder Structures

1. **adapters/**: Directory for implementing data adapters such as RDBMS for multi-database support.
2. **bin/**: Directory for storing binary tools such as mockery or go-migrate binary.
3. **cmd/**: Contains commands implemented for Cobra command line tools.
4. **docs/**: Contains generated Swagger code.
5. **drivers/**: Initialization to connect third-party drivers/providers like HTTP, message broker, and observability.
   - **http/**: Driver for HTTP and API implementation.
   - **messaging/**: Driver for messaging (NATS used, extendable to Kafka, Pulsar).
     - **nats/**: NATS message broker driver implementation.
6. **examples/**: Files for the main package where you can run the application and extend features.

7. **gorm/**: Model files for GORM.
8. **infrastructure/**: Infrastructure for applications such as database, message broker, API requests, etc. If you want to add multi-DB features, you can add them in the `adapters/register.go` file and follow the example.
   - **messaging/**: Messaging infrastructure.
     - **nats/**: Infrastructure for NATS (You can change this, if you have another message broker provider).
   - **persistent/**: Persistent infrastructure such as database or files.
9. **internal/**: Core application components and orchestration.
10. **migrations/**: Migration files.
11. **mocks/**: Generated mock files for Mockery tools (DO NOT EDIT).
12. **pkg/**: Public packages; you can add custom packages here.
13. **repositories/**: Repository implementations for use_case service interface repository.
14. **seeders/**: Seeder files.
15. **tools/**: Helper tools for applications.
16. **use_cases/**: Main logic and implementation files for applications. Every use case must contain this folder:
    - **dto/**: Data Transfer Object.
    - **port/**: Input and output for Hexagonal Architecture; this folder contains only interfaces.
      - **input/**
        - **message/**:
          - **listener/**
      ...
      - **output/**
        - **messaging/**
        - **repository/**
17. **protobuf/**: Protobuf files for gRPC or Messaging Schema.

---

## Rules

1. ## Save time in db must be converted to int64

## Migration

This migration uses go-migrate as a dependency. Refer to the [Documentation](https://github.com/golang-migrate/migrate) for installation.

1. Create migration:

```shell
make generate-migration NAME=add_mood_to_users
```
...
### Generate DTO for Use Case

```shell
make compile-proto PROFO_FOLDER=orders PROTO_FILE=order_payload
```
...
Ensure understanding of NATS basics like Ephemeral and Durable Consumers.

### Cheat Sheet

Refer to this [Cheat Sheet for NATS](https://docs.nats.io/nats-server/configuration)
...
- **Function:** Starts NATS server with default configuration.

2. **Starting NATS Server with Custom Configuration**

   ```bash
   nats-server -c /path/to/config.conf
   ```

   - **Function:** Starts NATS server with provided configuration from `config.conf` file.
...
3. **Specifying Port**
   ...
   - **Function:** Starts NATS server on custom port, e.g., port `4222`.

4. **Starting NATS Server with Logging**
   ```bash
   nats-server -DV
   ```
   - **Function:** Starts NATS server with verbose debug logging (`-DV` for debug/verbose mode).

### nats CLI

1. **Publish Message to Subject**

   ```bash
   nats pub subject_name "message to be sent"
   ```

   - **Function:** Publishes a message to `subject_name`.
...
2. **Subscribe to Subject**
   ...
   - **Function:** Subscribes to messages from `subject_name`.
...
3. **Wildcard Subscription**
   ...
   - **Function:** Subscribes to messages from all subjects matching `subject.*`.

4. **View Server Information**

   ```bash
   nats conns
   ```

   - **Function:** Displays information about active connections to NATS server.
...
5. **View Subscriber Information**
   ...
   - **Function:** Lists active subscribers.

6. **View Subject Information**
   ```bash
   nats pub -s
   ```
   - **Function:** Lists available subjects.

### JetStream CLI (js)

JetStream enhances NATS with durable pub/sub and persistent message storage. Here are some JetStream CLI commands:

1. **Create Stream**

   ```bash
   nats stream create --config stream_config.json
   ```

   - **Function:** Creates a JetStream stream based on configuration in `stream_config.json`.

2. **Publish Message to Stream**

   ```bash
   nats publish subject_name "message to be sent"
   ```

   - **Function:** Publishes a message to a JetStream stream with subject `subject_name`.

3. **Subscribe to Stream**

   ```bash
   nats subscribe subject_name
   ```

   - **Function:** Subscribes to messages from a JetStream stream with subject `subject_name`.

4. **Consume Message from Stream (Consumer)**

   ```bash
   nats consumer next consumer_name
   ```

   - **Function:** Retrieves the next message from JetStream consumer `consumer_name`.
...
5. **View Stream Information**
   ...
   - **Function:** Displays information about JetStream stream `stream_name`.

6. **View Consumer Information**
   ```bash
   nats consumer info stream_name consumer_name
   ```
   - **Function:** Displays information about JetStream consumer `consumer_name` in stream `stream_name`.

### Additional Notes

- Replace placeholders like `subject_name`, `stream_config.json`, `consumer_name`, and `stream_name` with appropriate values for your application.
- Use `nats-server -h` and `nats -h` for additional options available for `nats-server` and `nats` CLI.
- Ensure the NATS server you run supports JetStream for using JetStream CLI commands.
...
This guide was originally cloned from [Uber Style Guide](https://github.com/uber-go/guide/blob/master/style.md)
...
- [staticcheck](https://staticcheck.dev) to do various static analysis check
...
- Sort by alphabetical order in order to make the addition of new entities as
  ...
  - Within an `import` of multiple names from a module, sort the names in
    ...
    ```javascript
    import { CREATE_TICKET_TYPE } from "./actionTypes";
    ```
